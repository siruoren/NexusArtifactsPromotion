...
| 功能 | 描述 |
|------|------|
| **制品晋级** | 将制品从源仓库复制到目标仓库（HTTP 下载→上传），支持单文件/目录晋级，MD5 增量跳过，失败重试+指数退避，Chunked 流式传输，Maven Metadata 智能合并，文件列表分页（10条/页） |
| **Proxy 仓库同步** | 从远程同步 Proxy 仓库缓存，支持单文件/目录/全库同步，每次全量同步，失败重试+指数退避 |
| **Docker 镜像晋级** | 解析 Docker Manifest 智能识别依赖的 Blob，仅晋级必需的 Blob 层，支持全量/指定 Tag，Chunked 流式+失败重试 |
| **Docker Proxy 同步** | 解析 Docker Manifest 后按依赖同步 Blob，避免无效传输，支持全量/指定 Tag，Registry V2 API 错误处理+重试 |
...
| **安全加固** | 仓库配置 HTTP 认证，敏感信息自动脱敏，任务状态持久化支持重启恢复 |
...
│  │  │ • Docker release repos     │  │  • Max sync queue size        ││    │
