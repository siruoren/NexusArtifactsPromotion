...
| 功能 | 描述 |
|------|------|
| **制品晋级** | 将制品从源仓库复制到目标仓库（HTTP 下载→上传），支持单文件/目录晋级，MD5 增量跳过，失败重试+指数退避，Chunked 流式传输，Maven Metadata 智能合并，文件列表分页（10条/页），队列容量限制防 OOM，搜索 API 分页全量获取 |
| **Proxy 仓库同步** | 从远程同步 Proxy 仓库缓存，支持单文件/目录/全库同步，失败重试+指数退避 |
| **Docker 镜像晋级** | 解析 Docker/OCI Manifest 智能识别依赖的 Blob，支持多架构（Fat Manifest/OCI Index）镜像晋级，仅晋级必需的 Blob 层，支持全量/指定 Tag，Chunked 流式+失败重试 |
| **Docker Proxy 同步** | 解析 Docker/OCI Manifest 后按依赖同步 Blob，支持多架构镜像同步，Docker 与通用 Sync 流程分离，支持全量/指定 Tag，Registry V2 API 错误处理+重试，远程优先路径探测（自动判断目录前缀/镜像/Tag），目录前缀模式支持同步子目录下所有镜像 |
| **同步队列页面** | 左侧菜单入口，登录即可见（无需管理员权限），支持按仓库/路径/状态/用户筛选，分页浏览（20条/页），状态显示为失败/进行中/取消/完成，鼠标悬停显示完整内容 |
| **定时同步任务** | 在 Nexus System → Tasks 中创建定时任务，自动同步 Proxy 仓库 |
| **可配置重试策略** | 通过 Capability UI 配置重试参数（基础延迟、最大延迟），指数退避+抖动，错误分类（仅重试瞬态错误） |
| **安全加固** | Per-connection SSL 证书信任（不影响 JVM 全局），SSRF 防护（X-Forwarded-Host 白名单校验 + 本地主机校验），仓库配置 HTTP 认证，敏感信息自动脱敏，全局异常处理规范化 |
| **任务队列管理** | 可配置任务队列最大记录数、任务日志过期自动清理（TTL）、任务超时自动取消（双层保护：Watchdog+定时扫描），任务终止状态一致性保障，服务重启后不恢复旧任务 |
| **HTTP 连接池复用** | JDK Keep-Alive 连接池（maxConnections=20），核心晋级/Sync/Docker 传输路径复用连接，减少 TLS 握手开销，Chunked 流式传输 |
| **单元测试** | DockerManifestParser 21 项测试覆盖 Docker V2/OCI/Fat Manifest/OCI Index/自动检测/边界情况 |
...
│  │  │  • Target repo filtered by edit+delete permission            │  │    │
│  │  └───────────────────────────┬───────────────────────────────────┘  │    │
│  └──────────────────────────────┼──────────────────────────────────────┘    │
│                                 │ REST API                                 │
│  ┌──────────────────────────────┼──────────────────────────────────────┐    │
...
│  │  │ • HTTP upload     │  │ • Cache delete   │  │ • Queue manage │ │    │
│  │  │ • MD5 compare     │  │ • NegCache inval │  │ • Dedup        │ │    │
│  │  │ • Search assets   │  │ • Docker deleg.  │  │ • Capacity ctrl│ │    │
│  │  └────────┬──────────┘  └───────┬──────────┘  └───────┬────────┘ │    │
│  │  ┌────────▼──────────┐  ┌───────▼──────────┐                          │    │
│  │  │  DockerService    │  │SyncQueuePersistMgr│                          │    │
│  │  │ • Manifest parse  │  │ • Cleanup on start│                          │    │
│  │  │ • Blob reference  │  │ • No task recover │                          │    │
│  │  │ • Image/tag list  │  └──────────────────┘                          │    │
│  │  │ • Docker promote  │                                                 │    │
│  │  │ • Docker sync     │                                                 │    │
│  │  └────────┬──────────┘                                                 │    │
...
│  │  │  └──────────────────┘  └──────────────┘  └────────────────┘  │    │
│  │  │  ┌──────────────────┐  ┌──────────────┐  ┌────────────────┐  │    │
│  │  │  │DockerManifestPsr │  │RetryableOp   │  │PromoExceptMap  │  │    │
│  │  │  │ • OCI support    │  │ • Exp. backoff│  │ • Error std.   │  │    │
│  │  │  │ • Fat manifest   │  │ • Error class│  │ • JSON format  │  │    │
│  │  │  └──────────────────┘  └──────────────┘  └────────────────┘  │    │
│  │  └───────────────────────────────────────────────────────────────┘    │
│  └──────────────────────────────────────────────────────────────────────┘    │
│                                                                             │
...
│  │  │ • Max promotion queue size │  │  • Max sync queue size        ││    │
│  │  │ • Retry base/max delay     │  │  • Max task queue records     ││    │
│  │  │ • Docker release repos     │  │  • Task log TTL / timeout     ││    │
│  │  └────────────────────────────┘  │  • Docker release proxy repos ││    │
│  │                                  └────────────────────────────────┘│    │
│  └──────────────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
用户点击 [Promote] 按钮
        ...
        ▼
┌───────────────────┐
│ 1. 权限检查        │  PermissionChecker.checkTargetPromotionPermission()
│    目标仓库编辑+删除 │  → 失败: 显示权限错误提示
└────────┬──────────┘
         │ 通过
         ▼
┌───────────────────┐
...
│    Proxy 仓库删除权 │  (定时任务跳过此步，前端按钮仅 proxy 仓库显示)
...
3. 选择目标 Docker 仓库
4. 确认执行，弹窗显示晋级文件列表状态（每个文件的路径及成功/失败状态）
...
| 操作 | 所需权限 | 说明 |
|------|----------|------|
| 制品晋级 | `repository-view:edit` + `repository-view:delete` (目标仓库) | 晋级需要上传文件（edit）和删除文件（delete）权限；目标仓库列表按同类型格式匹配，排除远程库（proxy），仅显示用户有 edit+delete 权限的仓库 |
| Proxy 同步 | `repository-view:delete` (Proxy 仓库) | 需要仓库的删除权限（因为要先删缓存再同步）；同步按钮仅对 proxy 类型仓库显示 |
| 同步队列 | 登录认证即可 | 所有已登录用户均可查看同步队列，无需管理员权限 |
...
│   ├── SyncRequest.java
│   ├── SyncTaskInfo.java
│   ├── DockerImageRequest.java
│   ├── DockerImageInfo.java
│   ├── DockerImageListResponse.java
│   └── ...
...
├── service/                # 业务逻辑
│   ├── PromotionService.java      # 通用制品晋级（含 Maven Metadata 合并）
│   ├── SyncService.java           # Proxy 仓库同步
│   ├── DockerService.java         # Docker 镜像晋级/同步
│   ├── TaskExecutorService.java   # 线程池管理
│   ├── TaskCacheManager.java      # 任务缓存管理
│   ├── TaskStateStore.java        # 任务状态持久化（JSON→磁盘）
│   ├── RetryableOperation.java    # 失败重试+指数退避
│   └── MavenMetadataMerger.java   # Maven Metadata 智能合并
└── task/                   # 定时任务
    ├── ProxySyncTask.java
...
- **安全**: Apache Shiro (Nexus 集成)，仓库配置 HTTP 认证
...
`RetryableOperation` 提供统一的失败重试机制：

| 参数 | 默认值 | 说明 |
|------|--------|------|
| 制品晋级重试 | 3 次 | 每个文件传输失败后自动重试 |
| Docker 晋级重试 | 3 次 | Blob/Manifest 传输失败后自动重试 |
| 同步操作重试 | 2 次 | ViewFacet.dispatch 失败后自动重试 |
| 退避策略 | 指数退避 | 1s → 2s → 4s（含随机抖动，上限 30s） |

**不可重试场景**：HTTP 400/401/403/404（客户端错误）、`IllegalArgumentException`（参数错误）

### 2. 流式传输优化

大文件传输使用 `Transfer-Encoding: chunked`（64KB 分块）：
- **内存优化**：避免将整个文件缓冲到内存（Docker Blob 可达 GB 级别）
- **连接优化**：无需预先计算 Content-Length，边读边写
- **超时控制**：每个 chunk 的读写受连接超时保护

### 3. Maven Metadata 智能合并

晋级 `maven-metadata.xml` 时自动合并源和目标：

| 元素 | 合并策略 |
|------|----------|
| `versioning/versions` | 取并集，保留所有版本号 |
| `versioning/latest` | 取最高版本号（语义化版本比较） |
| `versioning/release` | 取最高非 SNAPSHOT 版本 |
| `versioning/lastUpdated` | 取最新时间戳 |
| `snapshotVersions` | 按 classifier+extension 去重，保留最新 |

### 4. 任务状态持久化

`TaskStateStore` 将已完成的任务状态持久化到磁盘（JSON 格式）：
- 存储路径：`{karaf.data}/promotion-task-state/promotion/` 和 `sync/`
- 启动时自动加载：Nexus 重启后用户仍可查询历史任务结果
- TTL 清理：30 分钟后自动清理过期的任务状态文件

### 5. 认证机制

| 措施 | 说明 |
|------|------|
| 仓库 HTTP 认证 | 使用仓库配置的 HTTP 认证信息访问远程仓库 |
| 敏感信息脱敏 | 错误消息中的 password/token/secret 自动替换为 `***` |
| 无需额外凭据 | 移除了管理员账户密码配置，简化部署和维护 |

### 6. Docker Registry API v2 错误处理
