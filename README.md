...
| 功能 | 描述 |
|------|------|
| **制品晋级** | 将制品从源仓库复制到目标仓库（HTTP 下载→上传），支持单文件/目录晋级，MD5 增量跳过，失败重试+指数退避，Chunked 流式传输，Maven Metadata 智能合并，文件列表分页（10条/页），队列容量限制防 OOM |
| **Proxy 仓库同步** | 从远程同步 Proxy 仓库缓存，支持单文件/目录/全库同步，每次全量同步，失败重试+指数退避，活跃任务持久化支持重启恢复 |
...
| **安全加固** | 仓库配置 HTTP 认证，敏感信息自动脱敏，任务状态持久化支持重启恢复，全局异常处理规范化 |
...
│  │  │ • Manifest parse  │  │ • Active task per.│                          │    │
│  │  │ • Blob reference  │  │ • Startup recover │                          │    │
│  │  │ • Image/tag list  │  └──────────────────┘                          │    │
│  │  │ • Docker promote  │                                                 │    │
│  │  │ • Docker sync     │                                                 │    │
│  │  └────────┬──────────┘                                                 │    │
...
│  │  │ • Retry base/max delay     │  │  • Max sync queue records     ││    │
│  │  │ • Docker release repos     │  │  • Docker release proxy repos ││    │
│  │  └────────────────────────────┘  └────────────────────────────────┘│    │
│  └──────────────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────────┘
```
