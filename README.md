...
| **Proxy 仓库同步** | 从远程同步 Proxy 仓库缓存，支持单文件/目录/全库同步，MD5 增量比对，失败重试+指数退避 |
...
| **安全加固** | 管理员凭据 AES-256-GCM 加密存储，敏感信息自动脱敏，任务状态持久化支持重启恢复 |
...
│   └── CredentialEncryptor.java   # AES-256-GCM 凭据加密
├── service/                # 业务逻辑
...
- **安全**: Apache Shiro (Nexus 集成) + AES-256-GCM 凭据加密
...
| 措施 | 说明 |
|------|------|
| AES-256-GCM 加密 | 管理员密码使用 ENC: 前缀加密存储 |
| PBKDF2 密钥派生 | 基于实例特征派生密钥，不可跨实例解密 |
| 敏感信息脱敏 | 错误消息中的 password/token/secret 自动替换为 `***` |
| 向后兼容 | 未加密的明文凭据仍可使用，更新时自动加密 |
