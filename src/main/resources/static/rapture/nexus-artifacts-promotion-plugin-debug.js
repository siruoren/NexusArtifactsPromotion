...
function getCsrfToken() {
  // Nexus stores CSRF token in a cookie, must send as header on POST/PUT/DELETE
  // The token is generated by NX.app.Application if not present
  try {
    var token = Ext.util.Cookies.get('NX-ANTI-CSRF-TOKEN');
    if (token) return token;
  } catch (e) { /* ignore */ }
  // Fallback: parse document.cookie
  var match = document.cookie.match(/NX-ANTI-CSRF-TOKEN=([^;]+)/);
  if (match) return match[1];
  // Last resort: generate and set the cookie (same as NX.app.Application does)
  var newToken = Math.random().toString();
  try {
    var basePath = window.location.pathname.substring(0, window.location.pathname.indexOf('/', 1)) || '';
    Ext.util.Cookies.set('NX-ANTI-CSRF-TOKEN', newToken, null, basePath, window.location.hostname);
  } catch (e) {
    document.cookie = 'NX-ANTI-CSRF-TOKEN=' + newToken + '; path=/';
  }
  return newToken;
}
...
function apiRequest(method, path, data) {
  ...
  // Add CSRF token for non-GET requests (same as Ext.Ajax default headers)
  var csrfToken = getCsrfToken();
  if (csrfToken && method !== 'GET') {
    headers['NX-ANTI-CSRF-TOKEN'] = csrfToken;
  }
  var opts = {
    method: method,
    headers: headers,
      ...
      if (!response.ok) {
        return response.text().then(function (text) {
          try {
            ...
            if (parseErr.message !== 'No permission' && !parseErr.message.startsWith('401')) {
